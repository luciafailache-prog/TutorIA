...
export async function generateExplanation(apiKey: string, topic: string, theory: string, difficulty: string, messages: { role: string, text: string }[]) {
...
export async function correctExercise(apiKey: string, imageUri: string, exerciseContent: string) {
  const ai = getAI(apiKey);
  const prompt = `
...
export async function generateMockExam(apiKey: string, baseExam: string, topic: string, theory: string) {
  const ai = getAI(apiKey);
  const prompt = `
...
export async function proposeExercise(apiKey: string, topic: string, theory: string, existingExercises: any[]) {
  const ai = getAI(apiKey);
  const prompt = `
...
export async function generateSchematic(apiKey: string, exercisePrompt: string) {
  const ai = getAI(apiKey);
